...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  # transformed.
  # ---- contrib lib no longer supported in TF 2.x ----
  # if quantize_layer:
  #   if is_training:
  ...
  with tf.compat.v1.name_scope('train'):
    if FLAGS.Adam == 'True':
      print("------------------ADAM----------------------")
      optimizer = tf.compat.v1.train.AdamOptimizer(FLAGS.learning_rate, FLAGS.momentum)
...
def main(_):
  ...
  with tf.compat.v1.Session(graph=graph) as sess:
    ...
    if FLAGS.train_from_ckpt != None:
        ...
        tf.compat.v1.train.Saver().restore(sess, FLAGS.train_from_ckpt)
    else:
